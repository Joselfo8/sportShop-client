...
// Styles
import style from "./OrderProgress.module.css";
...
export default function OrderProgress() {
  ...
  function validate(data: any) {
    ...
    if (!data.trackingNumber) {
      ...
      errors.trackingNumber =
        "Tracking number must not contain symbols or special characters";
    }
  ...
  const handleOnSubmit: any = (event: any) => {
    ...
    if (
      input.selectState === "Order ready" &&
      unduplicatedStates.includes("Preparing order")
    ) {
    ...
    if (
      input.selectState === "Order is on its way" &&
      unduplicatedStates.includes("Order ready")
    ) {
    ...
    if (
      input.selectState === "Order delivered" &&
      unduplicatedStates.includes("Order is on its way")
    ) {
  ...
  let statusHistory = !order.status_history
    ? null
    : order.status_history.map((state: any) => state.status);

  const unduplicatedStates = !order.status_history
    ? []
    : possibleStates.filter((state: any) => {
        if (statusHistory.includes(state) === false) return state;
      });

  const rederSelectState = !order.id ? (
    <div>
      <h2>Loadding...</h2>
    </div>
  ) : (
      ...
      <div className={style.ordersContainer}>
        <div className={style.orderBar}>
          ...
          <div className={style.secondLine}>
            ...
            <p>
              Quantity: <b>{`${`${quantityOfProducts} products`}`}</b>
            </p>
          </div>
        </div>
        ...
        <div className={style.orderStatusContainer}>
          ...
          <p className={style.textCenter}>
            {order.status_history[order.status_history.length - 1].date}
          </p>
              ...
              <div className={style.textSymple}>
                ...
                <div className={style.flexForm}>
                  <form
                    action=""
                    id="orderNumber"
                    onSubmit={(e) => addTrackingNumber(e)}
                  >
                    <input
                      name="trackingNumber"
