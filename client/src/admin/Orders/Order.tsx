...
export default function Order() {
  ...
  let total: number = !order.id
    ? 0
    : order.sub_total + order.taxes + order.send_cost;
  let quantityOfProducts: number = !order.id
    ? 0
    : order.products
        .map((product: any) => Number(Object.values(product.sizesAmount)))
                  ...
                  <div className={style.productsContainer} key={product.id}>
                    ...
                    <div className={style.productPrice}>
                      ...
                      <div className={style.Subtotal}>
                        <p>{`$ ${
                          product.price *
                          Number(Object.values(product.sizesAmount))
                        }`}</p>
                      </div>
                    </div>
                  </div>
