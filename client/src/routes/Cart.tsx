...
export default function Cart() {
  ...
  let amountProduct: any =
    state.products && state.products.length
      ? state.products
          .map(
            (e: any) =>
              Object.values(e.sizesAmount)
                .map((e) => Number(e))
                .reduce((e: any, d: any) => e + d) * e.price
          )
          .reduce((z: number, y: number) => z + y)
      : 0;
        ...
        <div className={styles.centralice}>
                      ...
                      <table>
                        ...
                        <tbody>
                          <tr>
                            ...
                            <td>
                              {Object.values(e.sizesAmount).map(
                                (key: any, index) => (
                                  <Div key={index}>{key}</Div>
                                )
                              )}
          ...
          <div className={styles.half2}>
            ...
            <Link
              ...
              style={{ width: "100%" }}
            >
              <button className={styles.buttonBuy} onClick={handleAdress}>
                BUY
              </button>
            </Link>
          </div>
