...
const stripePromise = loadStripe(
  "pk_test_51LKaEAATR7GdGLkc7mu5mssziGvjyttaMQtfXseG4I9kS4EBvdgPLm67UpkkRQ13I1UWUe7JjVUWMalVudbwbkl000KbydKI9L"
);
...
export default function Purchase() {
  ...
  const subTotal =
    state.shoppinglist.list && state.shoppinglist.list.length > 0
      ? state.shoppinglist.list
          .map((p: any) => p.price * Number(Object.values(p.sizesAmount)[0]))
  ...
  const soldProducts =
    state.shoppinglist.list &&
    state.shoppinglist.list.map(
      (p: any, index: number) => `${index + 1}- ${p.title} $${p.price}`
    );
  ...
  const render =
    state.shoppinglist.list && state.shoppinglist.list.length > 0 ? (
          ...
          <div className={style.product} key={product.id}>
            <div className={style.image}>
              <img
                src={
                  product.image ||
                  "https://th.bing.com/th/id/R.4f2b0468bf3ad3a7ab079eb1b219b27e?rik=rW45jIXlLLnMNQ&pid=ImgRaw&r=0"
                }
                alt="Camisa"
              />
            </div>
            ...
            <div className={style.price}>
              ...
              <div className={style.subtotalPrice}>
                <p>{`$${
                  product.price * Number(Object.values(product.sizesAmount)[0])
                }`}</p>
              </div>
            </div>
          </div>
      ...
      <div className={style.warningContainer}>
        ...
        <h2 className={style.withoutProducts}>
          You have not selected products to buy!
        </h2>
      </div>
    );
      ...
      <div className={style.purchaseContainer}>
        ...
        <div className={style.shoppingBox}>
          <h4 className={style.subtitle}>
            Payment method & purchase confirmation
          </h4>
            ...
            <Elements stripe={stripePromise}>
              <div className="formContainer">
                <CheckoutForm
                  ...
                  direction={
                    state.userInformation.shippingAddresses[0].firstAddress
                  }
